class DataStore:
    ...
    def __init__(self, share_keys=None, isolate_keys=None, shared_dict=None):
        ...
        # 其余key默认隔离
        self.clients = {}  # client_id -> client实例
    ...
    def auto_register_strategies(self):
        """
        自动调用所有通过@decorator注册的策略注册函数。
        """
        try:
            from .DataStoreStrategies import DATASTORE_STRATEGY_REGISTRY
        except ImportError:
            return
        for reg_func in DATASTORE_STRATEGY_REGISTRY:
            reg_func(self)
