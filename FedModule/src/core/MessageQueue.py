...
class DataGetter(Thread):
    ...
    def run(self) -> None:
        ...
        while not self.is_end:
            # 检查数据是否可用
            if self.message_queue.is_data_available():
                # 当数据可用，处理队列中的所有数据
    ...
    def kill(self):
        self.is_end = True
        # 确保数据被标记为可用，以便线程能够退出等待状态
        self.message_queue.set_data_available()
...
class MessageQueue:
    ...
    __data_available_lock = mp.Lock()

    @staticmethod
    def set_data_available():
        """设置数据可用标志"""
        with MessageQueue.__data_available_lock:
            MessageQueue.__data_available = True

    @staticmethod
    def clear_data_available():
        """清除数据可用标志"""
        with MessageQueue.__data_available_lock:
            MessageQueue.__data_available = False

    @staticmethod
    def is_data_available():
        """检查数据是否可用"""
        with MessageQueue.__data_available_lock:
            return MessageQueue.__data_available

    @staticmethod
    def get_from_uplink(key='update'):
        with MessageQueue.__uplink_lock:
            if MessageQueue.uplink[key].qsize() == 1:  # 取出最后一条数据后，重置事件状态
                result = deepcopy(MessageQueue.uplink[key].get())
                MessageQueue.clear_data_available()
                return result
    ...
    def put_into_uplink(item, key='update'):
        with MessageQueue.__uplink_lock:
            ...
            MessageQueue.set_data_available()
