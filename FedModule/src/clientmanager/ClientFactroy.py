...
class ClientFactory:
    ...
    @staticmethod
    def create_client(c_id, stop_event, selected_event, client_staleness, index, client_config, dev, total_config=None):
        # 统一单 client 创建接口，index 传单个
        if not total_config:
            total_config = GlobalVarGetter.get()['config']
        mode, params = running_mode(total_config)
        client_class = ModuleFindTool.find_class_by_path(total_config["client"]["path"])
        ...
        data_proxy = global_var.get('data_proxy', None)
        try:
            client = client_class(c_id, stop_event, selected_event, client_staleness, index, client_config, dev, data_proxy=data_proxy)
        except TypeError:
            client = client_class(c_id, stop_event, selected_event, client_staleness, index, client_config, dev)
