...
class DistributedClientManager(BaseClientManager):
    def __init__(self, whole_config):
        ...
        # 新增client_events支持
        self.client_events = self.config.get("client_events", [])
        self.all_client_ids = set()
        self.initial_active_ids = self._get_initial_active_ids()
        self.client_num = len(self.initial_active_ids)
        ...
        for event in self.client_events:
            if event.get('round') == current_round:
                for cid in event.get('join', []):
                    # 通知sub-manager创建并启动client
                    self.create_and_start_new_client(client_id=cid)
                for cid in event.get('leave', []):
                    self.stop_client_by_id(cid)

    def create_and_start_new_client(self, client_id=None, dev='cpu'):
        # 通知sub client-manager创建并启动client
        # 这里假设client_id能唯一定位到sub-manager
        if client_id is None:
            return
        # 计算属于哪个sub-manager
        sub_id = 0
        prefix = 0
        for i, num in enumerate(self.config["client_num"]):
            if client_id < prefix + num:
                sub_id = i
                break
            prefix += num
        # 发送创建client的消息
        self.communication_proxy.send(sub_id, "manager_affair_create_client", client_id)
...
class SubNormalClientManager(NormalClientManager):
    def __init__(self, whole_config):
        ...
        # 监听主manager的创建client指令
        self.communication_proxy.get(self.manager_id, "manager_affair_create_client", self._on_create_client)

    def _get_initial_active_ids(self):
        join_ids = set()
        for event in self.client_events:
            join_ids.update(event.get('join', []))
        return [self.start_id + i for i in range(self.total_client_num) if (self.start_id + i) not in join_ids]

    def _on_create_client(self, client, userdata, message):
        import pickle
        client_id = pickle.loads(message.payload)
        self.create_and_start_new_client(client_id=client_id)

    def create_and_start_new_client(self, client_id=None, dev=None):
        # 支持指定client_id创建
        if client_id is None:
            client_id = self.start_id + self.client_num
        rel_id = client_id - self.start_id
        if rel_id < 0 or rel_id >= self.total_client_num:
            return
        if not hasattr(self, 'client_list') or self.client_list is None:
            self.client_list = [None] * self.total_client_num
        if self.client_list[rel_id] is None:
            self._create_client(rel_id)
        if rel_id not in self.client_id_list:
            self.client_id_list.append(rel_id)
        self.client_list[rel_id].start()
        self.client_num += 1

    def _create_client(self, rel_id):
        # rel_id为本地相对id
        client_id = self.start_id + rel_id
        self.client_list[rel_id] = self.client_factory.create_client(
            client_id, self.stop_event_list[rel_id],
            self.selected_event_list[rel_id],
            self.client_staleness_list[rel_id], self.index_list[rel_id],
            self.client_config, self.client_dev[rel_id], self.global_var['config'])
